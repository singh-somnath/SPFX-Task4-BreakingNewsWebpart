...
export class GridLayout extends React.Component<IGridLayoutProps, IGridLayoutState> {
   ...
   */
  private _onRenderCell = (item: any, index: number): JSX.Element => { // eslint-disable-line @typescript-eslint/no-explicit-any
    const isCompact: boolean = this._isCompact;
    let cellPadding:number =0;
    if(index<this._columnCount)
    {
        cellPadding = index !== this._columnCount - 1 ? PADDING : 0;
    }
    else
    {
        cellPadding = index % this._columnCount !== this._columnCount - 1 ? PADDING : 0;
    }
